resource "aws_launch_configuration" "podtatohead-arms" {
  ...
  user_data = base64encode(templatefile("${path.module}/templates/init.tpl", { name="${var.podtato_name}-arms", container_image = "ghcr.io/fhb-codelabs/podtato-small-arms", podtato_version=var.podtato_version, left_version=var.left_arm_version, right_version=var.right_arm_version, dd_api_key=var.datadog_api_key} ))
